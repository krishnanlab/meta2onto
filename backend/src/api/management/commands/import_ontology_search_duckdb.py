    ...
    def handle(self, *args, **opts):
        ...
        if opts["clear_existing"]:
            with transaction.atomic():
                self.stdout.write(
                    self.style.WARNING("Clearing existing ontology search data...")
                )

                models_to_clear = (OntologySearchDocs, OntologySynonyms, OntologyTerms)
        ...
        rows = con.execute(
            "SELECT term_id, ontology, type, name, syn_exact, syn_narrow, syn_broad, syn_related FROM ontology_search_docs"
        ).fetchall()
        ...
        rows = con.execute(
            "SELECT term_id, synonym, scope FROM ontology_synonyms"
        ).fetchall()
        ...
        rows = con.execute(
            "SELECT id, name, ontology, type FROM ontology_terms"
        ).fetchall()
