...
class Command(BaseCommand):
    ...
    def _import_to_model(
        ...
        if verbose:
            self.stdout.write(
                self.style.HTTP_INFO(
                    f"→ Importing {src_table} into {model.__name__} with chunk size {chunk_size}, batch size {batch_size}..."
                )
            )
        ...
        with tqdm(
            total=total_rows, desc=f"Importing {model.__name__}", unit="rows"
        ) as pbar:
            while True:
                ...
                with transaction.atomic():
                    ...
                    objs = [
                        model(**dict(zip(cols, _truncate_cells(row))))
                        for row in rows_chunk
                    ]

                    model.objects.bulk_create(
                        objs, ignore_conflicts=True, batch_size=batch_size
                    )
